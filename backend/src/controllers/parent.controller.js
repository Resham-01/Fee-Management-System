...
const unlinkChildSchema = Joi.object({
...
// Parent: Unlink/remove child from parent account
exports.unlinkChild = async (req, res) => {
  try {
    const { error, value } = unlinkChildSchema.validate(req.body);
    if (error) {
      return res.status(400).json({ message: error.details[0].message });
    }
    ...
    logger.error('Unlink child error', { error: err.message });
    res.status(500).json({ message: 'Server error' });
  }
};
